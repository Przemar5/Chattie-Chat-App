...
class Database
{
	...
	public function buildSelectQuery(string $table, $data)
	{
		...
		try
		{
			...
			$this->_query .= $this->_getOrderQueryPart($order ?? '');
	...
	private function _getOrderQueryPart($order = '')
	{
		if (!is_string($order))
		{
			throw new \Exception("Order must be passed as a 'string', not" . 
								 gettype($order) . ".");
		}
		
		return (!empty(trim($order))) ? ' ORDER BY ' . trim($order) : '';
	}
